...
if($_GET['videoid']) {
	$config  = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."webplayer_videos WHERE id=".$_GET['videoid']);
} else {
	$config  = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."webplayer_videos WHERE playlistid=".$_GET['playlistid']);
}
$count   = count($config);
