...
function webplayer_plugin_shortcode( $atts ) {
	...
	$player = $wpdb->get_row("SELECT * FROM webplayer WHERE id=".$atts['id']);
	...
	if($player->videoid) {
		$results = $wpdb->get_row("SELECT * FROM webplayer_videos WHERE id=".$player->videoid);
	} else if ($player->playlistid) {
		$results = $wpdb->get_row("SELECT * FROM webplayer_videos WHERE playlistid=".$player->playlistid." LIMIT 1");
	}
